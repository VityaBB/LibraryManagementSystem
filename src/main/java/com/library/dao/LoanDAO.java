...
public class LoanDAO {
    ...
    public void updateLoan(Loan loan) throws SQLException {
        String sql = "UPDATE loans SET book_id = ?, user_id = ?, loan_date = ?, due_date = ?, return_date = ?, status = ?, fine_amount = ? WHERE id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, loan.getBookId());
            stmt.setInt(2, loan.getUserId());
            stmt.setDate(3, java.sql.Date.valueOf(loan.getLoanDate()));
    ...
    public List<User> getUsersByBookId(int bookId) throws SQLException {
        String sql = "SELECT u.id, u.email, u.password_hash, u.first_name, u.last_name, u.phone, u.address, u.registration_date, u.is_active, u.role FROM users u JOIN loans l ON u.id = l.user_id WHERE l.book_id = ? AND l.status IN ('ACTIVE', 'OVERDUE')";
    ...
    public List<Book> getActiveBooksByUserId(int userId) throws SQLException {
        String sql = "SELECT b.id, b.title, b.isbn, b.publication_year, b.publisher_id, b.total_copies, b.page_count, b.description FROM books b JOIN loans l ON b.id = l.book_id WHERE l.user_id = ? AND l.status IN ('ACTIVE', 'OVERDUE')";
