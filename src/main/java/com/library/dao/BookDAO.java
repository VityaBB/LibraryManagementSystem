...
public class BookDAO {
    ...
    public List<Book> searchBooks(String title, String author, String genre, int page, int pageSize) throws SQLException {
    ...
    public void updateBook(Book book) throws SQLException {
        String sql = "UPDATE books SET title = ?, isbn = ?, publication_year = ?, " +
                     "publisher_id = ?, total_copies = ?, page_count = ?, description = ? WHERE id = ?";
    ...
    public int getAvailableCopies(int bookId) throws SQLException {
        String sql = "SELECT (b.total_copies - (SELECT COUNT(*) FROM loans l WHERE l.book_id = b.id AND l.status IN ('ACTIVE', 'OVERDUE'))) AS available FROM books b WHERE b.id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, bookId);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
